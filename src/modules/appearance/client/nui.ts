...
export function registerNuiCallbacks(): void {
	...
	on("__cfx_nui:appearance_rotate_camera", (direction: "left" | "right", cb: (arg: any) => void): void => {
		cb({});
		rotateCamera(direction);
	});

	on("__cfx_nui:appearance_change_model", async (model: string, cb: (arg: any) => void): Promise<void> => {
	...
	on("__cfx_nui:appearance_change_component", (component: PedComponent, cb: (arg: any) => void): void => {
		const playerPed = PlayerPedId();
	...
	on("__cfx_nui:appearance_change_head_blend", (headBlend: PedHeadBlend, cb: (arg: any) => void) => {
		cb({});
		setPedHeadBlend(PlayerPedId(), headBlend);
	});

	on("__cfx_nui:appearance_change_face_feature", (faceFeatures: PedFaceFeatures, cb: (arg: any) => void): void => {
		cb({});
		setPedFaceFeatures(PlayerPedId(), faceFeatures);
	});

	on("__cfx_nui:appearance_change_head_overlay", (headOverlays: PedHeadOverlays, cb: (arg: any) => void): void => {
		cb({});
