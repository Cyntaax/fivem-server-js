...
import { pedModels, getPedAppearance, setPlayerAppearance } from "./appearance";

import { arrayToVector3 } from "../../../util";
...
export function getAppearanceSettings(): AppearanceSettings {
	...
	const headOverlays: HeadOverlaysSettings = HEAD_OVERLAYS.reduce((object: any, headOverlay: any, index: any) => {
		const settings = {
			style: {
				min: 0,
		...
		if (colorMap[headOverlay as keyof typeof colorMap]) {
			Object.assign(settings, {
				color: {
					items: colorMap[headOverlay as keyof typeof colorMap]
				}
			});
		}
...
export function setCamera(key: string): void {
	...
	const { coords, point } = CAMERAS[currentCamera as keyof typeof CAMERAS];

	const reverseFactor = reverseCamera ? -1 : 1;

	if (cameraHandle) {
		const camCoords = arrayToVector3(
			GetOffsetFromEntityInWorldCoords(PlayerPedId(), coords.x * reverseFactor, coords.y * reverseFactor, coords.z)
		);

		const camPoint = arrayToVector3(GetOffsetFromEntityInWorldCoords(PlayerPedId(), point.x, point.y, point.z));
		...
		const camCoords = arrayToVector3(GetOffsetFromEntityInWorldCoords(PlayerPedId(), coords.x, coords.y, coords.z));
...
export async function rotateCamera(direction: "left" | "right"): Promise<void> {
	...
	const { coords, point } = CAMERAS[currentCamera as keyof typeof CAMERAS];
	const offset = OFFSETS[currentCamera as keyof typeof OFFSETS];
