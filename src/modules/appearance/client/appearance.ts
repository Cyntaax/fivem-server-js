import { Delay, isPedFreemodeModel } from "../../../util";
...
function getPedHairDecoration(ped: number, hairStyle: number): HairDecoration {
	...
	const hairDecoration = HAIR_DECORATIONS[hairDecorationType].find((decoration) => decoration.id === hairStyle);
...
export function setPedHeadOverlays(ped: number, headOverlays: PedHeadOverlays): void {
	...
	HEAD_OVERLAYS.forEach((key, index) => {
		const headOverlay: PedHeadOverlayValue = headOverlays[key as keyof typeof headOverlays];
		...
		if (headOverlay.color || headOverlay.color === 0) {
			...
			if (isMakeupColor[key as keyof typeof isMakeupColor]) {
				colorType = 2;
			}
...
export function setPedComponent(ped: number, component: PedComponent): void {
	...
	if (excludedFromFreemodeModels[component_id as keyof typeof excludedFromFreemodeModels] && isPedFreemodeModel(ped)) {
		return;
	}
...
export async function setPlayerAppearance(appearance: PedAppearance): Promise<void> {
	...
	const { model, components, props, headBlend, faceFeatures, headOverlays, hair, eyeColor } = appearance;
