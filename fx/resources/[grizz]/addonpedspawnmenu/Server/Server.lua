...
---@param id string The steam id to check for admin
function CheckAdminDB(id)
	local p = promise:new()
	MySQL.Async.fetchAll("SELECT * FROM ped_users WHERE steam = @steam", {
		["@steam"] = id
	}, function(res)
		if res ~= nil then
			if #res > 0 then
...
AddEventHandler('AOPSM:CheckAdminStatus', function()
	...
	local out = Citizen.Await(CheckAdminDB(steamID))
	if out == true then
